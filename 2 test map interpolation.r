...
get_script_path <- function() {
  ...
  file_arg <- cmd_args[startsWith(cmd_args, "--file=")]
  if (length(file_arg) > 0) {
    return(normalizePath(
      normalize_slashes(sub("^--file=", "", file_arg[[1]])),
      winslash = "/",
      mustWork = FALSE
    ))
  }
  frame_paths <- Filter(
    ...
    lapply(sys.frames(), function(env) {
  ...
  if (length(frame_paths) > 0) {
    return(normalizePath(
      normalize_slashes(frame_paths[[length(frame_paths)]]),
      winslash = "/",
      mustWork = FALSE
    ))
  }
...
get_script_dir <- function() {
  ...
  if (dir.exists(script_path)) {
    return(normalize_slashes(script_path))
  }
  normalize_slashes(dirname(script_path))
}
ascend_paths <- function(path) {
  path <- normalizePath(normalize_slashes(path), winslash = "/", mustWork = FALSE)
  out <- character(0)
  repeat {
    out <- c(out, path)
    parent <- dirname(path)
...
score_project_root <- function(path) {
  ...
  if (dir.exists(file.path(path, "gps_records"))) {
    score <- score + 4L
  }
  if (dir.exists(file.path(path, "Python Maps"))) {
    score <- score + 2L
  }
  if (dir.exists(file.path(path, "Code"))) {
    score <- score + 1L
  }
  score
}
find_project_root <- function() {
  start_points <- unique(c(get_script_dir(), getwd()))
  candidates <- unique(unlist(lapply(start_points, ascend_paths), use.names = FALSE))
  scores <- vapply(candidates, score_project_root, integer(1))
  best_idx <- which.max(scores)
  if (length(best_idx) > 0 && scores[[best_idx]] > 0L) {
    return(normalize_slashes(candidates[[best_idx]]))
  }
  normalizePath(getwd(), winslash = "/", mustWork = FALSE)
}
first_path_or_fallback <- function(paths, fallback) {
  paths <- unique(normalize_slashes(paths))
  existing <- paths[vapply(paths, dir.exists, logical(1))]
  if (length(existing) > 0) {
    return(existing[[1]])
  }
  normalize_slashes(fallback)
}
# Derive defaults from the repository layout so the script stays portable
# across machines and fresh clones.
PROJECT_ROOT <- find_project_root()
DEFAULT_INPUT_DIR <- first_path_or_fallback(
  c(
    file.path(PROJECT_ROOT, "gps_records", "Google", "Interpolation output", "individual_interpolated_csvs"),
    file.path(PROJECT_ROOT, "data", "interpolated_csvs")
  ),
  file.path(PROJECT_ROOT, "data", "interpolated_csvs")
)
OUTPUT_BASE_DIR <- first_path_or_fallback(
  c(
    file.path(PROJECT_ROOT, "Python Maps"),
    file.path(PROJECT_ROOT, "outputs", "python_maps")
  ),
  file.path(PROJECT_ROOT, "outputs", "python_maps")
)
    ...
    stop(
      sprintf(
        paste(
          ...
          "Set --input_dir to your interpolated CSV folder or create that directory in the cloned project."
        ),
        cfg$input_dir
      ),
      call. = FALSE
