...
get_script_path <- function() {
  ...
  if (length(file_args) > 0) {
    return(normalizePath(sub("^--file=", "", file_args[[1]]), winslash = "/", mustWork = FALSE))
  }
...
read_env_flag <- function(name, default = TRUE) {
  ...
  stop(sprintf("Invalid boolean value for %s: %s", name, raw_value), call. = FALSE)
}

script_path <- get_script_path()
script_dir <- if (dir.exists(script_path)) script_path else dirname(script_path)
project_root <- normalizePath(file.path(script_dir, "..", "..", ".."), winslash = "/", mustWork = FALSE)
resolve_repo_path <- function(...) normalizePath(file.path(project_root, ...), winslash = "/", mustWork = FALSE)
...
config <- list(
  input_csv = normalizePath(
    Sys.getenv(
      "STATION_COMP_INPUT_CSV",
      unset = resolve_repo_path("Vergleichs Messtation", "mobile station passesver2.csv")
    ),
    winslash = "/",
    mustWork = FALSE
  ),
  output_dir = normalizePath(
    Sys.getenv(
      "STATION_COMP_OUTPUT_DIR",
      unset = resolve_repo_path("Line Graph Station Comp")
    ),
    winslash = "/",
    mustWork = FALSE
  ),
  clean_output_dir = read_env_flag("STATION_COMP_CLEAN_OUTPUT_DIR", default = TRUE),
...
if (!file.exists(config$input_csv)) {
  stop(
    paste(
      "Input CSV not found:",
      config$input_csv,
      "Set STATION_COMP_INPUT_CSV to override the default project-relative path."
    ),
    call. = FALSE
  )
}
          ...
          if (nrow(reducing_daily_summary) > 0) {
            ...
            if (is_silberstein_station(sid)) {
              reducing_daily_custom_svg <- file.path(
                config$output_dir,
                sprintf(
                  "%s_station_no_sensor_vs_mobile_reducing_gas_sensor_daily_line_compact.svg",
                  safe_filename(sid)
                )
              )
              ...
              reducing_daily_custom_meta <- save_daily_dual_axis_line_svg(
                ...
                station_series_source = "NO",
                left_axis_name = sprintf("Station NO (%s)", station_unit_label),
                title_text = sprintf("%s NO Sensor\nvs Mobile Reducing Gas Sensor", station_label_value),
          ...
          if (nrow(nox_daily_summary) > 0) {
            ...
            if (normalize_key(sid) == "mariendorferdamm") {
              oxidising_daily_custom_svg <- file.path(
                config$output_dir,
                sprintf(
                  "%s_nox_sensor_vs_mobile_oxidizing_gas_sensor_daily_line_compact.svg",
                  safe_filename(sid)
                )
              )
              ...
              oxidising_daily_custom_meta <- save_daily_dual_axis_line_svg(
                ...
                station_series_source = nox_source_label,
                left_axis_name = sprintf("%s (%s)", nox_series_name, station_unit_label),
                title_text = sprintf("%s %s Sensor\nvs. Mobile Oxidizing Gas Sensor.", station_label_value_nox, nox_series_name),
