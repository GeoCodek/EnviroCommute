...
def discover_repo_root(start: Path) -> Path:
    """
    Walk upward until we find the repository root or a folder that looks like it.
    """
    for candidate in (start, *start.parents):
        if (candidate / ".git").exists():
            return candidate
        if (candidate / "gps_records").exists() and (candidate / "Vergleichs Messtation").exists():
            return candidate
    return start
...
MOBILE_INPUT_FOLDER = (
    ...
    / "individual_interpolated_csvs"
)

# Output CSV path
OUTPUT_CSV = REPO_ROOT / "Vergleichs Messtation" / "mobile station passesver2.csv"

# Air-quality station CSV files
SILBER_STATION_CSV = (
    REPO_ROOT
    / "Vergleichs Messtation"
    / "Airq"
    / "Silbersteinstra\u00dfe Stundenwerte ber_mc144_20251104-20260304.csv"
)
MARIENDORF_STATION_CSV = (
    REPO_ROOT
    / "Vergleichs Messtation"
    / "Airq"
    / "Mariendorfer Damm Stundenwerte ber_mc124_20260101-20260328.csv"
)


def build_stations(
    silber_station_csv: str | Path = SILBER_STATION_CSV,
    mariendorf_station_csv: str | Path = MARIENDORF_STATION_CSV,
) -> list[dict]:
    """
    Build station metadata so callers can override only the CSV locations.
    """
    return [
        {
            ...
            "lon": 13.4415632,
            "radius_m": 50.0,
            "station_csv": Path(silber_station_csv),
        },
        {
            ...
            "lon": 13.38767323,
            "radius_m": 140.0,
            "station_csv": Path(mariendorf_station_csv),
        },
    ]
...
def slugify(text: str) -> str:
    ...
    replacements = {
        ...
        "Ü": "Ue",
        "ß": "ss",
        # Some upstream CSVs arrive with mojibake. Handle both forms.
        "Ã¤": "ae",
        "Ã¶": "oe",
        "Ã¼": "ue",
        "Ã„": "Ae",
...
def best_effort_name(path_like: str | Path) -> str:
    """
    Return a stable filename even when the incoming path string uses another OS style.
    """
    text = str(path_like).strip()
    if not text:
        return ""
    for path_cls in (PureWindowsPath, PurePosixPath, Path):
        name = path_cls(text).name
        if name and name != text:
            return name
    return text


def format_path_for_display(path_like: str | Path, repo_root: Path = REPO_ROOT) -> str:
    """
    Prefer repo-relative paths in logs so local machine details stay out of output.
    """
    path = Path(path_like)
    try:
        return str(path.relative_to(repo_root))
    except ValueError:
        return best_effort_name(path)


def resolve_repo_relative_path(path_like: str | Path, repo_root: Path = REPO_ROOT) -> Path:
    """
    Re-map old absolute paths from another clone back into the current repository.
    """
    text = str(path_like).strip()
    if not text:
        return Path(text)
    ...
    if not raw_path.is_absolute():
        repo_relative = (repo_root / raw_path).resolve(strict=False)
        if repo_relative.exists():
            return repo_relative

    for pure_cls in (PureWindowsPath, PurePosixPath):
        pure_path = pure_cls(text)
        parts = list(pure_path.parts)
        if repo_root.name in parts:
            anchor_index = parts.index(repo_root.name)
            rebased = repo_root.joinpath(*parts[anchor_index + 1 :])
            if rebased.exists():
                return rebased
...
def find_repo_file_by_name(file_name: str, repo_root: Path = REPO_ROOT) -> Path | None:
    """
    Fallback lookup for index files that store stale absolute paths.
    """
    if not file_name:
        return None

    likely_dirs = [
        repo_root / "air_records" / "Enviro_Out_Cut",
        repo_root / "air_records" / "Enviro_CSV_All",
        repo_root / "gps_records" / "Google" / "Interpolation output" / "individual_interpolated_csvs",
    ]
    for directory in likely_dirs:
...
def resolve_mapped_source_path(
    ...
    """
    Resolve a source CSV path from interpolation metadata in a clone-agnostic way.
    """
    resolved = resolve_repo_relative_path(source_file, repo_root=repo_root)
    if resolved.exists():
        return resolved

    file_name = source_name.strip() or best_effort_name(source_file)
    fallback = find_repo_file_by_name(file_name, repo_root=repo_root)
...
def load_interpolation_source_map(
    ...
    for idx_path in sorted(mobile_folder.glob(INTERPOLATION_INDEX_GLOB)):
        ...
        for row in idx.itertuples(index=False):
            ...
            mapping[best_effort_name(output_file)] = resolve_mapped_source_path(
...
def process_mobile_folder(
    ...
    source_map = load_interpolation_source_map(mobile_folder, repo_root=REPO_ROOT)
                ...
                if source_aq_path.exists():
                    try:
                        ...
                        skipped_files.append(
                            (
                                f"{mobile_file.name} (source: {best_effort_name(source_aq_path)})",
                    ...
                    skipped_files.append(
                        (
                            f"{mobile_file.name} (source: {best_effort_name(source_aq_path)})",
                            f"Mapped source file not found: {format_path_for_display(source_aq_path)}",
                        )
                    )
                    source_aq_cache[source_aq_path] = pd.DataFrame(columns=["mobile_aq_timestamp"])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Match interpolated mobile air-quality tracks to fixed monitoring stations.",
    )
    parser.add_argument(
        "--mobile-input-folder",
        type=Path,
        default=MOBILE_INPUT_FOLDER,
        help="Folder containing interpolated mobile CSV files.",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=OUTPUT_CSV,
        help="Destination CSV for matched mobile/station rows.",
    )
    parser.add_argument(
        "--silber-station-csv",
        type=Path,
        default=SILBER_STATION_CSV,
        help="Station CSV for Silbersteinstrasse.",
    )
    parser.add_argument(
        "--mariendorf-station-csv",
        type=Path,
        default=MARIENDORF_STATION_CSV,
        help="Station CSV for Mariendorfer Damm.",
    )
    parser.add_argument(
        "--pass-gap-seconds",
        type=int,
        default=PASS_GAP_SECONDS,
        help="Maximum gap between rows before splitting into a new pass when multi-pass mode is enabled.",
    )
    parser.add_argument(
        "--one-pass-per-day",
        dest="one_pass_per_day",
        action="store_true",
        default=ONE_PASS_PER_DAY,
        help="Assign at most one pass ID per station and local calendar day.",
    )
    parser.add_argument(
        "--multi-pass-per-day",
        dest="one_pass_per_day",
        action="store_false",
        help="Allow multiple pass IDs per station per day, split by the gap threshold.",
    )
    parser.add_argument(
        "--prefer-one-variant-per-base-log",
        dest="prefer_one_variant_per_base_log",
        action="store_true",
        default=PREFER_ONE_VARIANT_PER_BASE_LOG,
        help="Keep only the preferred interpolated variant for each base mobile log.",
    )
    parser.add_argument(
        "--keep-all-variants",
        dest="prefer_one_variant_per_base_log",
        action="store_false",
        help="Process every interpolated variant instead of collapsing to one per base log.",
    )
...
def main() -> pd.DataFrame:
    args = parse_args()
    stations = build_stations(
        silber_station_csv=args.silber_station_csv,
        mariendorf_station_csv=args.mariendorf_station_csv,
    )
    result = process_mobile_folder(
        mobile_folder=args.mobile_input_folder,
        stations=stations,
        output_csv=args.output_csv,
        pass_gap_seconds=args.pass_gap_seconds,
        one_pass_per_day=args.one_pass_per_day,
        prefer_one_variant_per_base_log=args.prefer_one_variant_per_base_log,
    )

    print(f"Done. Wrote {len(result):,} matched rows to:")
    print(format_path_for_display(args.output_csv))
    return result
