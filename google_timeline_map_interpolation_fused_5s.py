...
# USER SETTINGS
# ============================================================
PROJECT_ROOT_ENV_VAR = "ENVIRO_PI_PROJECT_ROOT"
SCRIPT_DIR = Path(__file__).resolve().parent


def _clean_optional_path_value(value: Optional[str]) -> Optional[str]:
    if value is None:
        return None
...
def _discover_project_root() -> Path:
    override = _clean_optional_path_value(os.getenv(PROJECT_ROOT_ENV_VAR))
    if override is not None:
        return Path(override).expanduser().resolve()

    search_roots = [SCRIPT_DIR, *SCRIPT_DIR.parents]
    for candidate in search_roots:
        if (candidate / "gps_records").exists() and (candidate / "Code").exists():
            return candidate
    for candidate in search_roots:
        if (candidate / "Code" / "GIS Model Routes").exists():
            return candidate

    fallback = SCRIPT_DIR.parent.parent
    return fallback if fallback.exists() else SCRIPT_DIR


PROJECT_ROOT = _discover_project_root()


def _repo_default_path(*parts: str) -> str:
    return str(Path(*parts))


def _path_setting_from_env(name: str, default: Optional[str]) -> Optional[str]:
    override = _clean_optional_path_value(os.getenv(name))
    if override is not None:
        return override
    return default
...
def _resolve_configured_path(value: Optional[str]) -> Optional[str]:
    text = _clean_optional_path_value(value)
    if text is None:
        return None
...
# Defaults are repo-relative so the script can be committed/shared safely.
# Set ENVIRO_PI_PROJECT_ROOT or pass CLI args if your data lives elsewhere.
GOOGLE_TIMELINE_CSV = _path_setting_from_env(
    "GOOGLE_TIMELINE_CSV",
    _repo_default_path("gps_records", "Google", "Zeitachse_flat.csv"),
)
...
# SENSOR_GPS_CSV = None
SENSOR_GPS_CSV = _path_setting_from_env(
    "SENSOR_GPS_CSV",
    _repo_default_path("air_records", "Enviro_Out_Cut"),
)
SENSOR_GPS_FILE_GLOB = "enviro_log*.csv"
SENSOR_FILE_HINT_PADDING_HOURS = 24.0

# None -> save next to the Google CSV unless overridden via CLI or env var.
OUTPUT_DIR: Optional[str] = _path_setting_from_env("OUTPUT_DIR", None)
...
GOOGLE_UNI_CLASSIFICATION_CSV = _path_setting_from_env(
    "GOOGLE_UNI_CLASSIFICATION_CSV",
    _repo_default_path("gps_records", "Google", "google_timeline_uni_route_classification.csv"),
)
UNI_ROUTE_SHAPE_DIR = _path_setting_from_env(
    "UNI_ROUTE_SHAPE_DIR",
    _repo_default_path("Code", "GIS Model Routes"),
)
NO_GOOGLE_UNI_CSV = _path_setting_from_env(
    "NO_GOOGLE_UNI_CSV",
    _repo_default_path("gps_records", "Google", "No_google_uni.csv"),
)
GPS_TRAINING_DIR = _path_setting_from_env(
    "GPS_TRAINING_DIR",
    _repo_default_path("gps_records", "GPS Training"),
)
OPNV_MODE_GPKG = _path_setting_from_env(
    "OPNV_MODE_GPKG",
    _repo_default_path("Code", "GIS Model Routes", "opnv_mode.gpkg"),
)
...
def select_uni_route_shapes_from_classification(
    ...
    if excluded_shapes:
        try:
            target_csv = Path(no_google_uni_csv).expanduser()
            ...
            print(f"[INFO] Wrote excluded route shapes to {target_csv}")
        except Exception as exc:
...
def parse_cli_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to flattened Google Timeline CSV. Relative paths resolve from the detected project root.",
    )
    parser.add_argument(
        "--sensor-gps-csv",
    ...
    parser.add_argument(
        "--output-dir",
        default=OUTPUT_DIR,
        help="Output directory. Use 'none' to write next to the Google CSV.",
    )
    parser.add_argument(
        "--uni-classification-csv",
        default=GOOGLE_UNI_CLASSIFICATION_CSV,
        help="Path to the optional UNI route classification CSV.",
    )
    parser.add_argument(
        "--uni-route-shape-dir",
        default=UNI_ROUTE_SHAPE_DIR,
        help="Directory containing the local UNI route shapefiles.",
    )
    parser.add_argument(
        "--no-google-uni-csv",
        default=NO_GOOGLE_UNI_CSV,
        help="CSV written with excluded route-shape filenames when classification filtering is enabled.",
    )
    parser.add_argument(
        "--gps-training-dir",
        default=GPS_TRAINING_DIR,
        help="Directory containing training CSVs for the optional intersection model.",
    )
    parser.add_argument(
        "--opnv-mode-gpkg",
        default=OPNV_MODE_GPKG,
        help="GeoPackage containing OPNV mode segments for route timing.",
    )
    return parser.parse_args(argv)


def _normalize_optional_path(value: Optional[str]) -> Optional[str]:
    return _clean_optional_path_value(value)


def validate_runtime_configuration() -> None:
    if not GOOGLE_TIMELINE_CSV:
        raise ValueError("No Google Timeline CSV configured. Pass --google-timeline-csv.")

    google_csv = Path(GOOGLE_TIMELINE_CSV).expanduser()
    if not google_csv.exists() or not google_csv.is_file():
        expected_rel = Path("gps_records") / "Google" / "Zeitachse_flat.csv"
        raise FileNotFoundError(
            "Google Timeline CSV not found: "
            f"{google_csv}. Pass --google-timeline-csv, set {PROJECT_ROOT_ENV_VAR}, "
            f"or place the file at '{expected_rel}' relative to the detected project root."
        )
...
def apply_cli_overrides(args: argparse.Namespace) -> None:
    ...
    global GOOGLE_UNI_CLASSIFICATION_CSV, UNI_ROUTE_SHAPE_DIR, NO_GOOGLE_UNI_CSV
    global GPS_TRAINING_DIR, OPNV_MODE_GPKG
    ...
    GOOGLE_TIMELINE_CSV = _resolve_configured_path(args.google_timeline_csv) or ""
    SENSOR_GPS_CSV = _resolve_configured_path(args.sensor_gps_csv)
    OUTPUT_DIR = _resolve_configured_path(args.output_dir)
    GOOGLE_UNI_CLASSIFICATION_CSV = _resolve_configured_path(args.uni_classification_csv) or ""
    UNI_ROUTE_SHAPE_DIR = _resolve_configured_path(args.uni_route_shape_dir) or ""
    NO_GOOGLE_UNI_CSV = _resolve_configured_path(args.no_google_uni_csv) or ""
    GPS_TRAINING_DIR = _resolve_configured_path(args.gps_training_dir) or ""
    OPNV_MODE_GPKG = _resolve_configured_path(args.opnv_mode_gpkg) or ""
    GTFS_RAIL_ZIP = None
