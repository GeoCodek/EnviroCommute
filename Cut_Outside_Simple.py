...
By default, the script expects the repository layout:
- air_records/Enviro_CSV_All
- air_records/Enviro_Out_Cut

Those defaults are resolved from the detected repo root. Override them with
--input-folder and --output-folder when needed.
"""
...
# Configuration
DEFAULT_INPUT_SUBDIR = Path("air_records") / "Enviro_CSV_All"
DEFAULT_OUTPUT_SUBDIR = Path("air_records") / "Enviro_Out_Cut"
DEFAULT_FILE_GLOB = "*.csv"
...
def find_project_root(start: Path | None = None) -> Path:
    """
    Find the nearest parent that looks like the repository root.
    Falls back to the script directory if no marker is found.
    """
    ...
    for candidate in (base_dir, *base_dir.parents):
        if (candidate / ".git").exists():
            return candidate
        if (candidate / "air_records").exists():
            return candidate

    return base_dir


def resolve_runtime_path(path_value: Path | None, repo_root: Path, default_subdir: Path) -> Path:
    """Resolve a user-supplied path or a repo-relative default path."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Crop Enviro CSV files using repo-relative defaults that are safe to share on GitHub."
        )
    )
    parser.add_argument(
        "--repo-root",
        ...
        help="Repository root used for default input/output folders.",
    )
    parser.add_argument(
        "--input-folder",
        type=Path,
        help=(
            ...
            "Defaults to <repo_root>/air_records/Enviro_CSV_All."
        ),
    )
    parser.add_argument(
        "--output-folder",
        type=Path,
        help=(
            "Folder for cropped CSV files. "
            "Defaults to <repo_root>/air_records/Enviro_Out_Cut."
        ),
    )
    parser.add_argument(
        "--file-glob",
        default=DEFAULT_FILE_GLOB,
        help=f"Glob for selecting input files. Default: {DEFAULT_FILE_GLOB}",
    )
    return parser.parse_args()


def process_files(input_folder: Path, output_folder: Path, file_glob: str = DEFAULT_FILE_GLOB) -> None:
    """Process all CSV files in input folder."""
    output_folder.mkdir(parents=True, exist_ok=True)

    input_files = sorted(input_folder.glob(file_glob))
...
def main() -> int:
    """Resolve runtime paths and process files."""
    args = parse_args()
    repo_root = args.repo_root.expanduser().resolve()
    input_folder = resolve_runtime_path(args.input_folder, repo_root, DEFAULT_INPUT_SUBDIR)
    output_folder = resolve_runtime_path(args.output_folder, repo_root, DEFAULT_OUTPUT_SUBDIR)

    if not input_folder.exists():
        logger.error(
            "Input folder does not exist: %s. Use --input-folder or --repo-root to override.",
            input_folder,
        )
        return 1
    ...
    process_files(input_folder=input_folder, output_folder=output_folder, file_glob=args.file_glob)
    return 0
